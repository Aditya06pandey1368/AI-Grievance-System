...
import { UserPlus, Mail, Lock, MapPin, ShieldCheck, User } from "lucide-react";
...
const OfficerManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Officer created & assigned to your department!");
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a] font-sans overflow-hidden transition-colors">
      ...
      <div className="pt-28 px-4 max-w-6xl mx-auto pb-12 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="group">
                        ...
                        <div className="relative">
                            ...
                            <input type="text" className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none transition-all" placeholder="e.g. Inspector Vijay" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                        </div>
                    </div>
                    {/* Email */}
                    <div className="group">
                        ...
                        <div className="relative">
                            ...
                            <input type="email" className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none transition-all" placeholder="officer@gov.in" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
                        </div>
                    </div>
                    {/* Password */}
                    <div className="group">
                        ...
                        <div className="relative">
                            ...
                            <input type="password" className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none transition-all" placeholder="••••••••" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
                        </div>
                    </div>
                    {/* Zones */}
                    <div className="group">
                        ...
                        <div className="relative">
                            ...
                            <input type="text" className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none transition-all" placeholder="Ward 1, Ward 2..." value={formData.zones} onChange={e => setFormData({...formData, zones: e.target.value})} />
                        </div>
                    </div>
                    ...
                    <Button 
                        ...
                        className={`w-full h-12 text-lg mt-4 shadow-lg shadow-primary-500/30 ${!isFormValid ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
            ...
            {/* PREVIEW CARD */}
            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="hidden lg:flex flex-col items-center justify-center relative perspective-1000">
                ...
                <motion.div whileHover={{ rotateY: 10, rotateX: 5 }} className="w-96 h-[500px] bg-gradient-to-br from-slate-900 to-slate-800 rounded-[2rem] p-6 text-white shadow-2xl relative overflow-hidden border border-slate-700/50">
                    ...
                    <div className="relative z-10 flex flex-col h-full items-center">
                         ...
                         <div className="text-center w-full space-y-2">
                             <h3 className="text-2xl font-bold tracking-tight">{formData.name || "Officer Name"}</h3>
                             <p className="text-primary-400 font-medium">Departmental Officer</p>
                         ...
                         <div className="w-full grid grid-cols-2 gap-4 mt-auto">
                            ...
                            <div className="bg-white/5 rounded-xl p-3 backdrop-blur-sm overflow-hidden">
                                ...
                                <p className="text-xs font-semibold truncate" title={formData.email}>{formData.email || "officer@gov.in"}</p>
                            </div>
                            <div className="col-span-2 bg-white/5 rounded-xl p-3 backdrop-blur-sm">
                                ...
                                <p className="text-sm font-semibold truncate">{formData.zones || "Unassigned"}</p>
                            </div>
                         </div>
                    </div>
                </motion.div>
