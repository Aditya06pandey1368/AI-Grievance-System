...
import { MapPin, Calendar, User, CheckCircle2, Clock, AlertTriangle, ArrowLeft, Building2, FileText, Activity } from "lucide-react";
...
const ComplaintDetail = () => {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#0f172a] text-slate-500">Loading...</div>;
  if (!data) return <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#0f172a] text-slate-500">Complaint not found.</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a] transition-colors font-sans pb-20">
      ...
      <div className="pt-24 px-4 max-w-6xl mx-auto">
        <Link to="/dashboard" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-indigo-600 mb-6"><ArrowLeft className="w-4 h-4" /> Back</Link>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase border ${data.priorityLevel === 'Critical' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>{data.priorityLevel} Priority</span>
              ...
              <div className="relative flex justify-between">
                {steps.map((step, index) => (
                  <div key={step} className="flex flex-col items-center gap-2">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 z-10 ${index <= currentStepIndex ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-300'}`}>
                      {index <= currentStepIndex ? <CheckCircle2 className="w-5 h-5" /> : <div className="w-2 h-2 rounded-full bg-slate-300" />}
                    </div>
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-400">{step.replace('_', ' ')}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-lg border dark:border-slate-700">
              ...
              <p className="text-slate-600 dark:text-slate-300 text-lg">{data.description}</p>
              <div className="mt-6 flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border dark:border-slate-700">
                <div className="p-3 bg-white dark:bg-slate-800 rounded-full shadow-sm text-indigo-500"><MapPin className="w-6 h-6" /></div>
                <div><p className="text-xs font-bold text-slate-400">Location</p><p className="font-bold text-slate-800 dark:text-white">{data.location}</p><p className="text-sm text-slate-500">Zone: {data.zone}</p></div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="space-y-8">
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-lg border dark:border-slate-700">
              <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-6">Assigned Officer</h3>
              {data.assignedOfficer ? (
                <div>
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-16 h-16 bg-indigo-100 rounded-2xl flex items-center justify-center text-indigo-600"><User className="w-8 h-8" /></div>
                    <div><p className="text-xl font-bold text-slate-900 dark:text-white">{data.assignedOfficer.name}</p><span className="inline-block px-2 py-0.5 rounded text-xs font-bold bg-green-100 text-green-700">Active Duty</span></div>
                  </div>
                ...
                <div className="text-center py-6"><p className="text-slate-500 font-medium">Pending Assignment</p><p className="text-xs text-slate-400">Waiting for department allocation</p></div>
              )}
            </motion.div>
              ...
              <div className="space-y-6 relative pl-2">
                ...
                {data.history?.map((h, i) => (
                  <div key={i} className="relative pl-8 group">
                    <div className="absolute left-0 top-1.5 w-6 h-6 rounded-full bg-white border-4 border-slate-200 group-hover:border-indigo-500 z-10"></div>
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-slate-400 uppercase mb-0.5">{new Date(h.timestamp).toLocaleDateString()}</span>
