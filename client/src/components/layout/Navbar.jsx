...
const Navbar = () => {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('user'); // Optional: remove user details if you store them
    navigate('/login');
    window.location.reload(); // Refresh to update UI state immediately
  };
      ...
      {isOpen && (
        ...
        >
          <div className="px-4 pt-2 pb-6 space-y-2">
            ...
            {/* Mobile Conditional Logic */}
            {user ? (
               <button 
                 onClick={handleLogout} 
                 className="block w-full text-left py-2 text-red-500 font-medium hover:bg-slate-100 dark:hover:bg-slate-800 rounded"
               >
                 Logout
               </button>
            ) : (
              <>
                ...
                <Link to="/register" className="block py-2 text-slate-600 dark:text-slate-300">Get Started</Link>
              </>
            )}
          </div>
        </motion.div>
